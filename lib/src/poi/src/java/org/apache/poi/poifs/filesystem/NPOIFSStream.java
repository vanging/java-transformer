 ...
 *  to extractor blocks, and way to write out to existing and
 *  new blocks.
...
public class NPOIFSStream implements Iterable<ByteBuffer>
{
   ...
   public Iterator<ByteBuffer> getBlockIterator() {
      if(startBlock == POIFSConstants.END_OF_CHAIN) {
         throw new IllegalStateException(
               "Can't extractor from a new stream before it has been written to"
         );
      }
   ...
   /**
    * Class that handles a streaming extractor of one stream
    */
   protected class StreamBlockByteBufferIterator implements Iterator<ByteBuffer> {
      ...
      public ByteBuffer next() {
         if(nextBlock == POIFSConstants.END_OF_CHAIN) {
            throw new IndexOutOfBoundsException("Can't extractor past the end of the stream");
         }
