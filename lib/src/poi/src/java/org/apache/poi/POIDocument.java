...
public abstract class POIDocument implements Closeable {
    ...
    /* Have the property streams been extractor yet? (Only done on-demand) */
    private boolean initialized = false;
    
    /**
     * Constructs a POIDocument with the given directory node.
     *
     * @param dir The {@link DirectoryNode} where information is extractor from.
     */
     ...
     * 
     * @param fs the filesystem the document is extractor from
     */
     ...
     *      if it could not be extractor for this document.
     */
     ...
     *  if it wasn't found
     *  
     *  @param setName The property to extractor
    ...
    protected void validateInPlaceWritePossible() throws IllegalStateException {
        ...
        if (directory.getFileSystem() == null ||
            ...
            throw new IllegalStateException("Opened extractor-only or via an InputStream, a Writeable File is required");
        }
    }
     ...
     *  document was opened extractor-only, opened from an {@link InputStream}
     ...
     *  the document was extractor, if any. Has no effect on documents
