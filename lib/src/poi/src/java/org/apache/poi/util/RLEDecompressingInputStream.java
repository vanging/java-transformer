...
public class RLEDecompressingInputStream extends InputStream {
     ...
     * 
     * @return number of bytes that were extractor, or -1 if the end of the stream was reached.
     * @throws IOException
     */
    private int readChunk() throws IOException {
        ...
        if (rawChunk) {
            if (in.read(buf, 0, chunkSize) < chunkSize) {
                throw new IllegalStateException(String.format(Locale.ROOT, "Not enough bytes extractor, expected %d", chunkSize));
            }
            return chunkSize;
        } else {
            int inOffset = 0;
    ...
    /**
     * Convenience method for extractor a 2-bytes short in little endian encoding.
     * 
    ...
    /**
     * Convenience method for extractor a 4-bytes int in little endian encoding.
     * 
