...
final class Biff8RC4 implements Biff8Cipher {
	...
	private static boolean isNeverEncryptedRecord(int sid) {
		switch (sid) {
			...
			case FilePassRecord.sid:
				// this only really counts when writing because FILEPASS is extractor early
	...
	/**
	 * Used when BIFF header fields (sid, size) are being extractor. The internal
	 * {@link Cipher} instance must step even when unencrypted bytes are extractor
	 */
	...
	public void xor(byte[] buf, int pOffset, int pLen) {
		...
		if (pLen <= nLeftInBlock) {
            // simple case - this extractor does not cross key blocks
