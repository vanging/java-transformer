...
public final class RecordFactoryInputStream {
	...
	private static final class StreamEncryptionInfo {
		 ...
		 * This will typically be the first or second record extractor. Possibly <code>null</code>
		 * if stream was empty
		 */
	 ...
	 * This is used at the start of the workbook stream, and also when the most recently extractor
	...
	/**
	 * @param in the InputStream to extractor from
	 * 
	...
	public RecordFactoryInputStream(InputStream in, boolean shouldIncludeContinueRecords) {
		...
		* non-zero padding that is extractor OK by Excel (Excel also fixes the padding).
		*
	 ...
	 * a recently extractor {@link MulRKRecord}. <code>null</code> if not present.
	 */
