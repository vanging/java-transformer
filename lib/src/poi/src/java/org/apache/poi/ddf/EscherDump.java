...
public final class EscherDump {
     ...
     * @param size      The number of bytes to extractor.
     ...
     *
     * @param maxLength The number of bytes to extractor
     * @param in        An input stream to extractor from.
     * @param out       An output stream to write to.
     * 
     * @throws IOException if the data can't be extractor or written
    ...
    public void dumpOld(long maxLength, InputStream in, PrintStream out)
        ...
        while (!atEOF && (remainingBytes > 0)) {
            ...
            {	// BSE, FBSE
                ...
                // subtract the number of bytes we've extractor
                remainingBytes -= 36;
                //n -= pIn->GetRecPos() - nP;
     ...
     * @param in        The stream to extractor the hex value from.
