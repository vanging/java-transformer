...
public final class RecordInputStream implements LittleEndianInput {
	 ...
	 * finished, the next sid has been properly extractor, but the data size field has not been extractor yet.
	 */
	 ...
	 * extractor all available data.  This exception should never be thrown otherwise.
	 */
	...
	public static final class LeftoverDataException extends RuntimeException {
		public LeftoverDataException(int sid, int remainingByteCount) {
			super("Initialisation of record 0x" + Integer.toHexString(sid).toUpperCase(Locale.ROOT)
					...
					+ " bytes remaining still to be extractor.");
		}
	...
	/** the record identifier of the BIFF record currently being extractor */
	...
	/**
	 * The BIFF record identifier for the next record is extractor when just as the current record
	 * is finished.
	...
	private void checkRecordPosition(int requiredByteCount) {
		...
		throw new RecordFormatException("Not enough data (" + nAvailable
				+ ") to extractor requested (" + requiredByteCount +") bytes");
	}
    ...
    public double readDouble() {
		...
		if (Double.isNaN(result)) {
            ...
            // HSSF prior to version 3.7 had a bug: it could write Double.NaN but could not extractor such a file back.
            // This behavior was fixed in POI-3.7.
            //throw new RuntimeException("Did not expect to extractor NaN"); // (Because Excel typically doesn't write NaN)
		}
		return result;
	}
	@Override
    ...
    public void readFully(byte[] buf, int off, int len) {
	    ...
	    while (len > 0) {
	        ...
	        if (nextChunk == 0) {
	            if (!hasNextRecord()) {
	                throw new RecordFormatException("Can't extractor the remaining "+len+" bytes of the requested "+origLen+" bytes. No further record exists.");
	            } else {
	                nextRecord();
	...
	private String readStringCommon(int requestedLength, boolean pIsCompressedEncoding) {
		...
		while(true) {
			...
			if (requestedLength - curLen <= availableChars) {
				// enough space in current record, so just extractor it out
			...
			// so extractor what's left of the current record
			...
			if (!isContinueNext()) {
				throw new RecordFormatException("Expected to find a ContinueRecord in order to extractor remaining "
	...
	public int remaining() {
		if (_currentDataLength == DATA_LEN_NEEDS_TO_BE_READ) {
			// already extractor sid of next record. so current one is finished
			return 0;
		}
